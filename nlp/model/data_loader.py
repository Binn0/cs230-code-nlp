...
class DataLoader(object):
    ...
    def __init__(self, data_dir, params):
        ...
        self.vocab = {} # key 存的是word，value存的是index
    ...
    def data_iterator(self, data, params, shuffle=False):
        ...
        order = list(range(data['size'])) # 一个包含数据索引的列表，用于决定数据的迭代顺序
        if shuffle: # 如果 shuffle 为 True，则打乱数据顺序。
            random.seed(230)
            random.shuffle(order)

        # one pass over data
        for i in range((data['size']+1)//params.batch_size): # 确定总批次数
            # fetch sentences and tags
            # 根据当前批次的索引，从数据集中提取句子和标签。
            ...
            # 找出当前批次中最长的句子的长度，以便对齐所有句子。
            batch_max_len = max([len(s) for s in batch_sentences])

            # prepare a numpy array with the data, initialising the data with pad_ind and all labels with -1
            # initialising labels to -1 differentiates tokens with tags from PADding tokens
            batch_data = self.pad_ind*np.ones((len(batch_sentences), batch_max_len)) # 使用填充值 pad_ind 初始化批次数据数组。
            batch_labels = -1*np.ones((len(batch_sentences), batch_max_len)) # 使用 -1 初始化批次标签数组，-1 表示填充标记

            # copy the data to the numpy array
            # 将每个句子和对应的标签复制到初始化的 numpy 数组中，确保每个句子都对齐到相同长度
            ...
            # 将 numpy 数组转换为 PyTorch 的 LongTensor。
            ...
            # 将张量转换为 PyTorch 的 Variable，以便在计算图中记录操作。
